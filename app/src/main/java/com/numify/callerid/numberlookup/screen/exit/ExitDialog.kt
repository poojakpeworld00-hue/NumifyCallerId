package com.numify.callerid.numberlookup.screen.exit
 ...
 * `MaterialAlertDialogBuilder` that HomeShellActivity used to build inline.
 *
 ...
 *
 * One live dialog at a time, same as [com.numify.callerid.numberlookup.access.fullscreen.FullScreenPrimingDialog].
 */
object ExitDialog {
    ...
    fun show(
        activity: Activity,
        cfg: OnboardingFlowConfig.ExitConfig,
        onExit: () -> Unit,
    ) {
     ...
     */
    private fun renderAd(activity: Activity, view: View, cfg: OnboardingFlowConfig.ExitConfig) {
        if (!cfg.dialogAdShow) return
        if (!AdsPrefStore.getInstance(activity).getBoolean("IsAdsON")) return
