...
class CallInsightViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun callsSubtitle(calls: List<CallRecord>): String {
        ...
        val onlyMissed = types.all { it == CallDirection.MISSED || it == CallDirection.SPAM }
        return when {
            types == setOf(CallDirection.OUTGOING) -> app.getString(R.string.detail_calls_outgoing)
            types == setOf(CallDirection.INCOMING) -> app.getString(R.string.detail_calls_incoming)
