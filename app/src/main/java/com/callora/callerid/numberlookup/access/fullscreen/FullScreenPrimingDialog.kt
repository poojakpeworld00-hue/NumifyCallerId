...
/**
 * The HomeShellActivity Full-Screen-Intent priming dialog. Same visual language as the
 * Screen (incoming-call preview + remote-driven copy). "Enable" opens the system
 * FSI page in-task (via HomeShellActivity's launcher) and arms [FullScreenWatchService]; the
 * dialog dismisses so the auto-return lands on a clean HomeShellActivity.
 *
...
object FullScreenPrimingDialog {
    ...
    fun show(
        ...
        view.findViewById<TextView>(R.id.fsDialogButtonVw).setOnClickListener {
            ...
            // only after that launch FSI settings in-task via HomeShellActivity's
            // launcher. The watcher + HomeShellActivity.onResume handle the return.
            enableTapped = true
            dialog.dismiss()
            AccessEngine.request(activity, "notification") {
                (activity as? HomeShellActivity)?.openFsiSettings()
            }
        }
