...
import com.callora.callerid.numberlookup.CipherVeil

/**
 * Credentials for the callerid.kpeworld.com API used by [LookupApi].
 *
...
object ApiSecrets {
    ...
    val API_ID: String by lazy { CipherVeil.s(BuildConfig.LOOKUP_API_ID) }

    /** hash_key query parameter. */
    val API_HASH: String by lazy { CipherVeil.s(BuildConfig.LOOKUP_API_HASH) }

    /** Authorization header value (already includes the "Bearer " prefix). */
    val API_TOKEN: String by lazy { CipherVeil.s(BuildConfig.LOOKUP_API_TOKEN) }
}
