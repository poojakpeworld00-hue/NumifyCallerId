...
import com.callora.callerid.numberlookup.databinding.CellSearchHistoryBinding
import com.callora.callerid.numberlookup.screen.shared.CallPresenter

class SearchTrailAdapter(
    private val onClick: (TrailEntry) -> Unit,
    private val onCall: (TrailEntry) -> Unit,
    ...
    private val onRevealName: (TrailEntry) -> Unit = {}
) : RecyclerView.Adapter<SearchTrailAdapter.VH>() {

    private val items = mutableListOf<TrailEntry>()
    ...
    fun submit(list: List<TrailEntry>) {
        items.clear()
        items.addAll(list)
    ...
    private fun isLocked(item: TrailEntry): Boolean =
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        with(holder.binding) {
            lblHistAvatar.text = CallPresenter.initials(item.name, item.rawNumber)
