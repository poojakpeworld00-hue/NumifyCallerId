...
import com.callora.callerid.adkit.policy.AdFrequencyManager.interBackCounter
...
class BackInterstitial {
    ...
    fun fetchBackInterstitials(activity: Activity) {
        ...
        if (AdSlotType.fromString(pref.getString("IsAdType")) == AdSlotType.GOOGLE) {
            InterstitialAd.load(
                activity, id, req,
        ...
        when (AdSlotType.fromString(pref.getString("IsAdType"))) {
            ...
            AdSlotType.CUSTOM, AdSlotType.UNKNOWN -> {
                if (pref.getBoolean("IsCustomADS"))
                    StandardInterstitial.launchDirectLink(act) { closeQuietly("custom_open") }
    ...
    private fun handleGoogleFail(
        ...
        if (pref.getBoolean("IsFail_FB")) {
            ...
            if (pref.getBoolean("IsCustomADS")) {
                StandardInterstitial.launchDirectLink(activity) { closeQuietly("google_fail_custom") }
    ...
    private fun showCustomAfterFBFail(
        ...
        if (pref.getBoolean("IsCustomADS"))
            StandardInterstitial.launchDirectLink(context) { closeQuietly("fb_fail_custom") }
