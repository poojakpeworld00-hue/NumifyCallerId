...
import com.numify.callerid.numberlookup.databinding.ScreenCallBackScreenBinding
import com.numify.callerid.numberlookup.kit.triggerClick
import com.numify.callerid.adkit.runtime.OpenAdManager
import com.numify.callerid.adkit.runtime.SheetNativeAds
import com.numify.callerid.adkit.runtime.SystemPromptHelper
import com.numify.callerid.adkit.runtime.getHD_VBC_Type
import com.numify.callerid.adkit.runtime.showcase.panes.NoticeFragment
import com.numify.callerid.adkit.runtime.showcase.panes.RecentCallListFragment
import com.numify.callerid.adkit.runtime.showcase.panes.ReminderListFragment
 ...
 * tabs (Message, ReminderItem, WhatsApp) and an ad slot. Extends the project's
 * [CoreActivity] so it picks up the standard DataBinding + locale/theme
 * plumbing.
 *
 * Note: the consent + Mobile Ads init that used to live here (via the
 * `getData(...)` call inherited from `AdHostActivity`) is expected to run
 * once during app startup. This screen only triggers ad rendering, not SDK
 * initialization.
 */
class AdShowcaseActivity : CoreActivity<ScreenCallBackScreenBinding>() {
    ...
    override fun initView() {
        ...
        if (getHD_VBC_Type() == "n") {
            ...
            SheetNativeAds().BS_showBigNative(this, binding.adContainerVw)
        } else {
            Log.w("987654321", "Banner called")
            SheetNativeAds().renderBannerAd(this, binding.adContainerVw)
        }
        ...
        val callerName = if (phone.isNotBlank() && !phone.equals("Private Number", ignoreCase = true))
            PhonebookSource(this).lookupNameByNumber(phone)?.takeIf { it.isNotBlank() }
        else null
        ...
        supportFragmentManager.beginTransaction()
            .replace(R.id.frag_container, RecentCallListFragment())
            .commit()
        ...
        onBackPressedDispatcher.addCallback(this) {
            ...
            when (currentFragment) {
                ...
                is RecentCallListFragment -> finish()
                else -> {
    ...
    private fun setupClickListeners() {
        ...
        val fragmentTabs = listOf(
            binding.picRecent to { RecentCallListFragment() as Fragment },
            binding.picMes to { NoticeFragment.newInstance(callerNumber) as Fragment },
            binding.picReminder to { ReminderListFragment() as Fragment }
        )
    ...
    private fun openWhatsAppApp() {
        ...
        for (pkg in packages) {
            ...
            try {
                ...
                Log.e("AdShowcaseActivity", "Failed to launch $pkg", e)
                // try next package
            }
        }
    ...
    override fun onDestroy() {
        super.onDestroy()
        isActive = false
        OpenAdManager.callbackshow = false
    }
