...
import com.numify.callerid.numberlookup.databinding.PaneContactsBinding
import com.numify.callerid.numberlookup.screen.HomeShellActivity
import com.numify.callerid.numberlookup.screen.insight.CallInsightActivity
import com.numify.callerid.numberlookup.kit.followAdContainer

class PhonebookFragment : CoreFragment<PaneContactsBinding>() {
    ...
    private val adapter = PhonebookAdapter(::dialNumber, ::openDetail)
    private val favoritesAdapter = FavoritesStripAdapter(::openDetail)
    ...
    override fun initView() {
        ...
        // Ad slot + dividers are handled by CoreFragment.showScreenAd() — see
        // CallHistoryFragment for why this no longer calls NativeBannerAd directly.
        ...
        binding.segAll.setOnClickListener { viewModel.setFilter(ContactScope.ALL) }
        binding.segFavorites.setOnClickListener { viewModel.setFilter(ContactScope.FAVORITES) }
        binding.segRecents.setOnClickListener { viewModel.setFilter(ContactScope.RECENTS) }
        binding.segGroups.setOnClickListener { viewModel.setFilter(ContactScope.GROUPS) }
    ...
    override fun initObservers() {
        ...
        viewModel.filter.observe(viewLifecycleOwner) { active ->
            showFavoritesStrip()
            highlightTab(binding.segAll, active == ContactScope.ALL)
            highlightTab(binding.segFavorites, active == ContactScope.FAVORITES)
            highlightTab(binding.segRecents, active == ContactScope.RECENTS)
            highlightTab(binding.segGroups, active == ContactScope.GROUPS)
        }
    ...
    private fun onPermissionGranted() {
        ...
        com.numify.callerid.numberlookup.engine.ContactSyncer
    ...
    private fun showFavoritesStrip() {
        val onAll = viewModel.filter.value == ContactScope.ALL
    ...
    private fun openDetail(contact: com.numify.callerid.numberlookup.store.PhonebookEntry) {
