...
import com.callora.callerid.numberlookup.access.fullscreen.FullScreenAccessActivity
import com.callora.callerid.numberlookup.screen.HomeShellActivity
import com.callora.callerid.numberlookup.screen.locale.LocalePickerActivity
import com.callora.callerid.numberlookup.screen.welcome.WelcomeActivity
 ...
 * [FullScreenAccess]/`FullScreenConfig` gate (grant-state + its own ledger),
 * rather than re-implemented here.
 */
object OnboardingFlowConfig {
    ...
    // --- Raw JSON access (stored as strings in AdsPrefStore, like ScreenAds) ---

    fun screenOrder(context: Context): List<String> {
        val raw = AdsPrefStore.getInstance(context).getString("screen_order", "[]") ?: "[]"
        return runCatching {
    ...
    private fun screenRoot(context: Context): JSONObject =
        runCatching {
            JSONObject(AdsPrefStore.getInstance(context).getString("screen", "{}") ?: "{}")
    ...
    fun exitConfig(context: Context): ExitConfig {
        val obj = runCatching {
            JSONObject(AdsPrefStore.getInstance(context).getString("exit", "{}") ?: "{}")
        }.getOrDefault(JSONObject())
     ...
     * [FullScreenAccess.shouldShowScreen] (its own grant-state + ledger are more
    ...
    fun isEligible(context: Context, key: String): Boolean {
        if (key == FSI_PERMISSION_KEY) return FullScreenAccess.shouldShowScreen(context)
    ...
    fun classFor(key: String): Class<*> = when (key) {
        LANGUAGE_KEY -> LocalePickerActivity::class.java
        ONBOARDING_KEY -> WelcomeActivity::class.java
        FSI_PERMISSION_KEY -> FullScreenAccessActivity::class.java
        else -> HomeShellActivity::class.java
    }
