...
import com.callora.callerid.numberlookup.screen.gate.FlowFooterAd
 ...
 * places `"fsi_permission"` — via [FullScreenAccess.shouldShowScreen]. Primes
 * the FSI grant then continues to the next eligible `screen_order` entry (see
 * [OnboardingFlowConfig.nextEligibleAfter]), or [HomeShellActivity] once none remain.
 *
...
class FullScreenAccessActivity : AppCompatActivity() {
    ...
    private val grantPoll = object : Runnable {
        override fun run() {
            ...
            if (FullScreenAccess.isGranted(this@FullScreenAccessActivity)) {
                EdgeInsets.log("FSI", "grant poll: GRANTED → continue to next")
                grantPolling = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (returningFromSettings || FullScreenAccess.isGranted(this)) {
            EdgeInsets.log("FSI", "Screen onCreate: returning/granted → continue (no render)")
            continueToNext()
            return
        }
        ...
        findViewById<TextView>(R.id.fsScreenButtonVw).setOnClickListener {
            ...
            AccessEngine.checkScreenPermissions(this, OnboardingFlowConfig.FSI_PERMISSION_KEY) {
        ...
        val autonextSec = OnboardingFlowConfig.stepConfig(this, OnboardingFlowConfig.FSI_PERMISSION_KEY)
            ?.autonextSec ?: 0
        if (autonextSec > 0) {
        ...
        FlowFooterAd.render(
            activity = this,
            screenKey = OnboardingFlowConfig.FSI_PERMISSION_KEY,
    ...
    private fun continueIfBackFromSettings(where: String) {
        ...
        if (returningFromSettings || granted) {
            EdgeInsets.log("FSI", "Screen $where: back from settings, granted=$granted → continue")
    ...
    private fun continueToNext() {
        ...
        val nextKey = OnboardingFlowConfig.nextEligibleAfter(this, OnboardingFlowConfig.FSI_PERMISSION_KEY)
        val nextClass = nextKey?.let { OnboardingFlowConfig.classFor(it) } ?: HomeShellActivity::class.java
        EdgeInsets.log("FSI", "Screen continueToNext → ${nextClass.simpleName}")
