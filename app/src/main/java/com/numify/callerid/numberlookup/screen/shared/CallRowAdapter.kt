...
import com.numify.callerid.numberlookup.store.CallLogRow
import com.numify.callerid.numberlookup.store.CallDirection
import com.numify.callerid.numberlookup.databinding.CellCallBinding

class CallRowAdapter(
    initial: List<CallLogRow> = emptyList(),
    private val onCall: (String) -> Unit = {},
    private val onIdentify: (String) -> Unit = {}
) : RecyclerView.Adapter<CallRowAdapter.VH>() {

    private var items: List<CallLogRow> = initial
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        val isSpam = item.type == CallDirection.SPAM
        ...
        with(holder.binding) {
            ...
            val (iconRes, subColorRes) = when (item.type) {
                CallDirection.INCOMING -> R.drawable.glyph_call_received to R.color.on_surface_variant
                CallDirection.OUTGOING -> R.drawable.glyph_call_made to R.color.on_surface_variant
                CallDirection.MISSED -> R.drawable.glyph_call_missed to R.color.danger
                CallDirection.SPAM -> R.drawable.glyph_warning to R.color.spam_on
            }
