...
class AdsPrefStore constructor(context: Context) {
    ...
    companion object {
        ...
        @Volatile
        private var instance: AdsPrefStore? = null

        fun getInstance(context: Context): AdsPrefStore {
            return instance ?: synchronized(this) {
                instance ?: AdsPrefStore(context.applicationContext).also {
                    instance = it
                }
            }
        }
         ...
         * called. Lets context-less callers (e.g. AccessRepository) read a
         * flag such as `OnMaketing` — it's initialised early in the launch flow.
         */
        fun getOrNull(): AdsPrefStore? = instance
    }
