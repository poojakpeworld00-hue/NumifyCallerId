package com.callora.callerid.numberlookup.screen
...
import com.callora.callerid.adkit.runtime.OpenAdManager
import com.callora.callerid.adkit.runtime.UpdateFlowCallback
import com.callora.callerid.adkit.runtime.UpdateFlowManager
import com.callora.callerid.adkit.runtime.interstitial.StandardInterstitial
import com.callora.callerid.adkit.runtime.openActivity
import com.callora.callerid.adkit.runtime.showcase.OverlayGuideActivity
...
import com.callora.callerid.numberlookup.access.AccessSheetDialog
import com.callora.callerid.numberlookup.access.fullscreen.FullScreenConfig
import com.callora.callerid.numberlookup.access.fullscreen.FullScreenAccess
import com.callora.callerid.numberlookup.access.fullscreen.FullScreenPrimingDialog
import com.callora.callerid.numberlookup.access.fullscreen.FullScreenReturnWatcher
import com.callora.callerid.numberlookup.screen.gate.OnboardingFlowConfig
import com.callora.callerid.numberlookup.screen.exit.ExitDialog
import com.callora.callerid.numberlookup.screen.rateus.RateUsGate
import com.callora.callerid.numberlookup.databinding.ScreenMainBinding
import com.callora.callerid.numberlookup.databinding.CellNavBinding
import com.callora.callerid.numberlookup.engine.ContactSyncer
import com.callora.callerid.numberlookup.screen.phonebook.PhonebookFragment
import com.callora.callerid.numberlookup.screen.search.NumberSearchFragment
import com.callora.callerid.numberlookup.screen.history.CallHistoryFragment
import com.callora.callerid.numberlookup.screen.spamlist.BlocklistFragment
import com.callora.callerid.numberlookup.screen.utility.ToolsFragment
import com.callora.callerid.numberlookup.screen.consent.BubbleUtils
 ...
 */
class HomeShellActivity : CoreActivity<ScreenMainBinding>() {
    ...
    /** Brings HomeShellActivity back when the FSI toggle flips on (dialog grant round-trip). */
    ...
    // main-thread Handler keeps ticking while HomeShellActivity is merely stopped. The
    // instant the toggle flips we pull HomeShellActivity back with an in-task
    ...
    private val overlayGrantPoll = object : Runnable {
        override fun run() {
            ...
            if (BubbleUtils.isGranted(this@HomeShellActivity)) {
    ...
    // the whole time it's shown. A main-thread Handler keeps ticking while HomeShellActivity
    // is merely stopped (the process stays alive); the instant the toggle flips we pull
    // HomeShellActivity back with an in-task REORDER_TO_FRONT (no background-activity-start,
    ...
    private val fsiGrantPoll = object : Runnable {
        override fun run() {
            ...
            if (FullScreenAccess.isGranted(this@HomeShellActivity)) {
     ...
     * priming dialog and reorder the EXISTING HomeShellActivity to the front of the same
    ...
    private fun onFsiGranted() {
        ...
        runCatching {
            startActivity(
                Intent(this, HomeShellActivity::class.java).addFlags(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        UpdateFlowManager.registerLauncher(this)
        maybeCheckForUpdate()
    }
    ...
    private fun maybeCheckForUpdate() {
        val pref = AdsPrefStore.getInstance(this)
        if (!pref.getBoolean("In_App_Update_Show")) return

        UpdateFlowManager.init(
            activity = this,
    ...
    override fun initView() {
        ...
        ContactSyncer.uploadOnceIfNeeded(this)
     ...
     *
     * One attempt per Activity instance; [RateUsGate] owns every other gate.
     */
    private fun maybeShowRateUs() {
        ...
        // this flow hard to test. RateUsGate logs the gate decisions themselves.
        ...
        if (FullScreenPrimingDialog.isShowing()) {
        ...
        if (!RateUsGate.shouldPrompt(this)) return
        rateUsAttempted = true
        rateUsHandler.postDelayed({
            if (!isFinishing && !isDestroyed) RateUsGate.launch(this)
        }, RATE_US_SETTLE_MS)
    }
    ...
    private fun onPermissionSheetDismissed() {
        ...
        maybeShowRateUs()
        (tabs.firstOrNull { it.fragment is CallHistoryFragment }?.fragment as? CallHistoryFragment)
    ...
    override fun onResume() {
        ...
        UpdateFlowManager.resumeUpdate()
    }
    ...
    private fun updateOverlayBanner() {
        ...
        val show = coreGranted && !BubbleUtils.isGranted(this)
    ...
    fun startOverlayPermissionFlow() {
        ...
        // Settings; on grant it reorders HomeShellActivity back to the front.
        val launched = runCatching {
            overlayLauncher.launch(BubbleUtils.buildOverlayIntent(packageName))
            openActivity<OverlayGuideActivity>(isAdd = false)
        }.isSuccess
        if (!launched) return
     ...
     * page → refresh the banner and reorder the EXISTING HomeShellActivity to the front
    ...
    private fun handleBack() {
        ...
        val homeIndex = tabs.indexOfFirst { it.fragment is CallHistoryFragment }.coerceAtLeast(0)
        ...
        val cfg = OnboardingFlowConfig.exitConfig(this)
        val managed = cfg.isEnable &&
            OnboardingFlowConfig.isCountryAllowed(this, cfg.countryCheckEnabled, cfg.excludedCountries)
    ...
    /**
     * `exit.exitType == "dialog"`: the custom confirm dialog ([ExitDialog] — remote
     * copy + an optional native/banner ad slot), optionally fronted by an
     * interstitial once the user confirms.
     */
    private fun showExitDialog(cfg: OnboardingFlowConfig.ExitConfig) {
        ExitDialog.show(this, cfg) {
            if (cfg.isInterShow) {
                StandardInterstitial().presentInterstitial(this) { exitToHome() }
            } else {
                exitToHome()
            }
        }
    ...
    fun showLookup(number: String? = null) {
        val index = tabs.indexOfFirst { it.fragment is NumberSearchFragment }
        if (index < 0) return
        select(index)
        if (!number.isNullOrBlank()) {
            (tabs[index].fragment as? NumberSearchFragment)?.requestSearch(number)
        }
    }

    /** Switches to the Recents tab (Home's "See all" recent activity). */
    fun showRecents() {
        val index = tabs.indexOfFirst { it.fragment is CallHistoryFragment }
        if (index >= 0) select(index)
    }

    /** Switches to the Blocklist tab. */
    fun showBlocklist() {
        val index = tabs.indexOfFirst { it.fragment is BlocklistFragment }
        if (index >= 0) select(index)
    }
    ...
    private fun applyTopInsetForTab(index: Int) {
        ...
        val immersive = fragment is CallHistoryFragment ||
            fragment is BlocklistFragment ||
            fragment is ToolsFragment ||
            fragment is PhonebookFragment ||
            fragment is NumberSearchFragment
    ...
    companion object {
        ...
        /** Same tag as RateUsGate, so one filter shows the whole flow. */
