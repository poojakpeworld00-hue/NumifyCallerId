package com.callora.callerid.numberlookup.core
...
import com.callora.callerid.adkit.policy.ScreenAdPlan
...
import com.callora.callerid.adkit.runtime.AdHostActivity
 ...
 * ```
 */
abstract class CoreActivity<DB : ViewDataBinding> : AdHostActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        applyLocale()
        applyTheme(LocalPrefs.selectedTheme(this).ifEmpty { THEME_LIGHT })
        ...
        updateNativeAdTheme(AdsPrefStore.getInstance(this), LocalPrefs.selectedTheme(this).ifEmpty { THEME_SYSTEM })
        ...
        // Note: forward/back interstitials are preloaded in AdHostActivity, so we
    ...
    protected fun goBack() {
        BackInterstitial().presentBackInterstitial(this) { performBack() }
    }
    ...
    protected open fun showBottomBanner() {
        ...
        ScreenAdPlan.showAd(this::class.java.simpleName, this, container, shimmer)
    ...
    override fun onResume() {
        super.onResume()
        applyLocale()
        StandardInterstitial.resumeAfterSettings(this)
        // NOTE: no LightHouse.syncPermissionsAsync() here — the SDK syncs
        // permissions internally (≥0.6.4), so an explicit call is redundant.
    }
    /** Applies the saved app language ([SettingsVault.languageTag]) via AppCompat. */
    protected fun applyLocale() {
        val savedLang = LocalPrefs.selectedLanguage(this)
    ...
    /** Applies the saved night-mode ([SettingsVault.themeMode]) app-wide. */
    ...
    private fun updateNativeAdTheme(adsPref: AdsPrefStore, theme: String) {
