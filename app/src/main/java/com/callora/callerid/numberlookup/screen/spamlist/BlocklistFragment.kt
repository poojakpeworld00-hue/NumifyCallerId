...
import com.callora.callerid.numberlookup.databinding.ScreenBlocklistBinding
import com.callora.callerid.numberlookup.databinding.SheetBlockAddBinding
import com.callora.callerid.numberlookup.databinding.SheetBlockDetailsBinding
import com.callora.callerid.numberlookup.databinding.SheetBlockRecentsBinding
import com.callora.callerid.numberlookup.databinding.SheetEnableCallerIdBinding
import com.callora.callerid.numberlookup.databinding.PartBlockAddFormBinding
import com.callora.callerid.numberlookup.kit.CallerIdController
...
class BlocklistFragment : CoreFragment<ScreenBlocklistBinding>() {
    ...
    private val countryPickerLauncher = registerForActivityResult(
        ...
        val dial = data.getStringExtra(RegionPickerActivity.EXTRA_DIAL) ?: return@registerForActivityResult
        val iso = data.getStringExtra(RegionPickerActivity.EXTRA_ISO).orEmpty()
        ...
        init {
            ...
            view.padCountryVw.setOnClickListener {
                ...
                countryPickerLauncher.launch(
                    Intent(requireContext(), RegionPickerActivity::class.java)
                )
            }
    ...
    private fun showContactsPicker() {
        val contacts = PhonebookSource(requireContext()).getContacts()
            .filter { it.detail.isNotBlank() }
            .map { CallRecord(it.name, it.detail, CallDirection.OUTGOING, 0L, 0L) }
    ...
    private fun showRecentsPicker() {
        val recents = CallLogSource(requireContext()).getCalls(limit = 200)
    ...
    private fun showPickDialog(
        ...
        val pickAdapter = BlockChooserAdapter { entry ->
            dialog.dismiss()
    ...
    private fun requireCallerId(action: () -> Unit) {
        if (CallerIdController.isCallerIdEnabled(requireContext())) {
            action()
        } else {
    ...
    private fun requestEnableCallerId() {
        val intent = CallerIdController.buildEnableIntent(requireContext()) ?: run {
    ...
    private fun refreshCallerIdGate() {
        if (view == null) return
        if (!CallerIdController.isCallerIdEnabled(requireContext())) return
