package com.callora.callerid.numberlookup.engine.incoming
...
import com.callora.callerid.numberlookup.screen.ringing.RingScreenActivity
 ...
 */
class CallerBubbleService : Service() {
    ...
    private val callEndWatcher by lazy { CallCloseWatcher(this) { stopSelf() } }
    ...
    private fun showOverlay(number: String) {
        ...
        val view = LayoutInflater.from(this).inflate(R.layout.bubble_caller_id, null)
        ...
        scope.launch {
            val info = withContext(Dispatchers.IO) { CallerBadge.resolve(this@CallerBubbleService, number) }
            overlayView?.let { CallerBadge.bind(this@CallerBubbleService, it, number, info) }
        }
    }
    ...
    companion object {
        ...
        fun start(context: Context, number: String) {
            val intent = Intent(context, CallerBubbleService::class.java)
                .putExtra(EXTRA_NUMBER, number)
            runCatching { context.startService(intent) }
        }

        fun stop(context: Context) {
            runCatching { context.stopService(Intent(context, CallerBubbleService::class.java)) }
        }
    }
}
