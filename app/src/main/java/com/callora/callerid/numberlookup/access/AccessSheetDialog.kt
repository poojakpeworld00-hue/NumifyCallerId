...
/**
 * Bottom-sheet replacement for HomeShellActivity's old sequential first-run
 ...
 *
 * Self-contained: it owns its own result launchers, so HomeShellActivity only has to
 ...
 * [BubbleUtils]. `phone_state` and `overlay` are listed unconditionally: they
...
class AccessSheetDialog : BottomSheetDialogFragment() {
    ...
    private val overlayLauncher = registerForActivityResult(
        ...
        val granted = context?.let { ctx -> BubbleUtils.isGranted(ctx) } ?: false
    ...
    private fun launchOverlay(finishAfter: Boolean) {
        // Reuse HomeShellActivity's overlay flow — it opens the system page with the
        ...
        val host = activity as? HomeShellActivity
        if (host != null) {
        ...
        // Fallback (not hosted by HomeShellActivity): own launcher, no auto-back, but
        ...
        runCatching {
            overlayLauncher.launch(BubbleUtils.buildOverlayIntent(requireContext().packageName))
            requireContext().openActivity<OverlayGuideActivity>(isAdd = false)
        }.onFailure {
            EdgeInsets.error("PermissionSheet", "Failed to open overlay settings", it)
            if (finishAfter) finishFlow()
        }
    }
    ...
    companion object {
        ...
        fun hasPending(activity: FragmentActivity): Boolean {
            ...
            if (!BubbleUtils.isGranted(activity)) return true
            return false
        }
         ...
         * screen: the engine records it in [AccessPrefs], the Home quick
         * actions in [SettingsVault]) and `shouldShowRequestPermissionRationale` is
        ...
        fun isPermanentlyDenied(activity: FragmentActivity, key: String, perm: String): Boolean {
            ...
            val askedAnywhere = AccessPrefs(activity).wasAsked(key) ||
                SettingsVault(activity).hasRequestedPermission(perm)
         ...
         * Combines [hasPending] with the shared [OnboardingFlowConfig] gate, driven
         * by the `screen.permission_sheet` entry of the Onboarding Dynamic Flow
         * (`isEnable` + `session` = every | once | `<N>` launches | `<N>d` days).
         * The ledger is stamped by [OnboardingFlowConfig.markShown] in [show]. This
        ...
        fun shouldAutoShow(activity: FragmentActivity): Boolean {
            ...
            return OnboardingFlowConfig.isEligible(activity, OnboardingFlowConfig.PERMISSION_SHEET_KEY)
        }
        ...
        fun show(activity: FragmentActivity, onFinished: (() -> Unit)? = null) {
            ...
            OnboardingFlowConfig.markShown(activity, OnboardingFlowConfig.PERMISSION_SHEET_KEY)
