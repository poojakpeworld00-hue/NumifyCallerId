 ...
 * The Screen appears once after Language; the Dialog appears in HomeShellActivity,
...
object FullScreenAccess {
    ...
    const val ACTION_FSI_GRANTED =
        "com.callora.callerid.numberlookup.action.FSI_GRANTED"
    ...
    // --- CountryEntry gate (reuses the app's existing IP + SIM country signals) ---
     ...
     * the feature is shown. Matches the IP country ([AdsPrefStore.userCountry]),
    ...
    fun isCountryAllowed(context: Context, config: FullScreenConfig): Boolean {
        if (!config.countryFilterEnabled || config.excludedCountries.isEmpty()) {
            EdgeInsets.log(TAG, "country: ALLOWED (filter off or empty list)")
            return true
        }
        val excluded = config.excludedCountries
        val signals = deviceCountrySignals(context)
        if (signals.isEmpty()) {
            EdgeInsets.log(TAG, "country: ALLOWED (no country signal → fail-open)")
            return true
        }
        val blocked = signals.any { sig -> excluded.any { it.equals(sig, ignoreCase = true) } }
        EdgeInsets.log(TAG, "country: ${if (blocked) "BLOCKED" else "ALLOWED"} [signals=$signals, excluded=$excluded]")
        return !blocked
    }
    ...
    private fun deviceCountrySignals(context: Context): List<String> {
        ...
        AdsPrefStore.getInstance(context).userCountry.takeIf { it.isNotBlank() }?.let { out += it.trim() }
        runCatching {
    ...
    fun shouldShowScreen(context: Context, config: FullScreenConfig = FullScreenConfig.load(context)): Boolean {
        if (!canRun(context, config) || !config.screen.enabled) {
            EdgeInsets.log(TAG, "SCREEN → NO (canRun failed or screen.enabled=${config.screen.enabled})")
            return false
        }
        val shown = SettingsVault(context).fsiScreenShown
        val result = if (config.screen.showOnce) !shown else true
        EdgeInsets.log(TAG, "SCREEN → ${if (result) "YES" else "NO"} [showOnce=${config.screen.showOnce}, alreadyShown=$shown]")
        return result
    }

    /** Dialog (HomeShellActivity): master gate + `dialog.enabled` + interval + max-count. */
    fun shouldShowDialog(context: Context, config: FullScreenConfig = FullScreenConfig.load(context)): Boolean {
        if (!canRun(context, config) || !config.dialog.enabled) {
            EdgeInsets.log(TAG, "DIALOG → NO (canRun failed or dialog.enabled=${config.dialog.enabled})")
            return false
        }
        val prefs = SettingsVault(context)
        val count = prefs.fsiDialogShowCount
        if (count >= config.dialog.maxShowCount) {
            EdgeInsets.log(TAG, "DIALOG → NO (count=$count >= max_show_count=${config.dialog.maxShowCount})")
            return false
        }
        val last = prefs.fsiDialogLastShownMs
        if (last == 0L) {
            EdgeInsets.log(TAG, "DIALOG → YES (first show; count=$count/${config.dialog.maxShowCount})")
            return true
        }
        val elapsedDays = (System.currentTimeMillis() - last) / (24L * 60L * 60L * 1000L)
        val result = elapsedDays >= config.dialog.showAfterDays
        EdgeInsets.log(TAG, "DIALOG → ${if (result) "YES" else "NO"} [elapsed=${elapsedDays}d, need=${config.dialog.showAfterDays}d, count=$count/${config.dialog.maxShowCount}]")
        return result
    }
    ...
    fun markScreenShown(context: Context) {
        SettingsVault(context).fsiScreenShown = true
        EdgeInsets.log(TAG, "ledger: screen marked shown")
    }
    ...
    fun markDialogShown(context: Context) {
        ...
        EdgeInsets.log(TAG, "ledger: dialog shown, count now ${prefs.fsiDialogShowCount}")
    }
     ...
     * `BubbleUtils.buildOverlayIntent`. The page is a `singleTask` Settings
    ...
    fun openSettings(activity: Activity, launcher: ActivityResultLauncher<Intent>) {
        ...
        if (launched) {
            EdgeInsets.log(TAG, "openSettings: FSI manage page launched in-task, watcher armed")
            FullScreenWatchService.start(activity)
        } else {
            EdgeInsets.error(TAG, "Failed to open FSI settings", null)
        }
    }
