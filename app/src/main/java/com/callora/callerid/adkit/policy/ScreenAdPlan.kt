package com.callora.callerid.adkit.policy
...
import com.callora.callerid.adkit.runtime.BannerLifecycleObserver
import com.callora.callerid.adkit.runtime.BannerAdLoader
import com.callora.callerid.adkit.runtime.BannerDimension
import com.callora.callerid.adkit.runtime.BannerVariant
 ...
 *
 * In DEBUG builds every resolution is logged under the tag `ScreenAdPlan`
 * (filter logcat by that tag to verify which id/type each screen uses).
 */
object ScreenAdPlan {

    private const val TAG = "ScreenAdPlan"
    ...
    fun resolve(context: Context, screenName: String): ScreenAd {
        ...
        // screen like HomeShellActivity (show:false) stays hidden while still using the
    ...
    fun showAd(
        ...
        // disableInternalFallback=true → BannerAdLoader reports a single onAdFailed()
        // so the native-banner fallback owns the failure path (no double-load).
        BannerAdLoader().renderBanner(
            activity = activity,
            container = container,
            ...
            observer = object : BannerLifecycleObserver {
                override fun onAdFailed() {
                    ...
                    NativeBannerAd().renderNativeBanner(activity, container, shimmer)
                }
            }
        )
    }
