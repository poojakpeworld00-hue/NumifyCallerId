package com.contacts.callerid.number.lookup.feature.splash
...
import com.contacts.callerid.number.lookup.monetize.model.ResultCallback
...
import com.contacts.callerid.number.lookup.monetize.delivery.showAppRedirectPopup
...
import com.contacts.callerid.number.lookup.databinding.ActivitySplashBinding
...
class SplashActivity : BaseActivity<ActivitySplashBinding>() {
    ...
    override fun initView() {
        ...
        // Watchdog: if the getData chain never calls back, force the splash
        // forward so it can't stall indefinitely.
        //
        // It was commented out, and a Firebase project swap proved why it should
        // not be: pointed at a project whose Remote Config template is empty,
        // getData fired neither callback and the splash sat on its progress bar
        // forever — no error, no way out but force-stop. Whatever leaves the
        // chain hung (an empty config, a hung native step, a dead network), an
        // app that never opens is the worst of the available failures.
        //
        // Safe to double up with the real callback: proceedNow() is guarded by a
        // compareAndSet, so whichever arrives second does nothing.
        handler.postDelayed({
            if (!proceeded.get()) {
    ...
    private fun launchNext() {
        ...
        // AppOpenAdManager (via ContactsApplication.handleAppForeground) and,
