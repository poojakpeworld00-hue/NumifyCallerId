package com.callora.callerid.adkit.runtime.interstitial
...
import com.callora.callerid.adkit.policy.AdFrequencyManager.interCounter
...
class StandardInterstitial {
    ...
    companion object {
        ...
        fun launchDirectLink(context: Activity, onClosed: () -> Unit) {
            val url = AdsPrefStore.getInstance(context).getString("DirectLink")
...
//                context.safeLog("browser_open_failed")
    ...
    fun fetchInterstitials(activity: Activity) {
        ...
        val adType = AdSlotType.fromString(pref.getString("IsAdType"))

        if (adType == AdSlotType.GOOGLE) {
            ...
            InterstitialAd.load(
                ...
                object : InterstitialAdLoadCallback() {
                    override fun onAdFailedToLoad(error: LoadAdError) {
                        ...
                        Log.e("StandardInterstitial", "Load Failed: ${error.message}")
                    }
                    override fun onAdLoaded(ad: InterstitialAd) {
                        googleInterAd = ad
                        activity.safeLog("google_inter_loaded")
                        Log.d("StandardInterstitial", "Google inter loaded")
                    }
                })
        }

        if (adType == AdSlotType.FACEBOOK || pref.getBoolean("IsFail_FB")) {
    ...
    private fun showAdInternal(activity: Activity?, adsClose: () -> Unit) {
        ...
        val pref = AdsPrefStore.getInstance(act)
        var hasClosed = false
    ...
    private fun loadAndShowGoogleOnDemand(
        ...
        InterstitialLoader.show(activity, isLoader)

        InterstitialAd.load(
            activity, id, AdRequest.Builder().build(),
            object : InterstitialAdLoadCallback() {
                override fun onAdLoaded(ad: InterstitialAd) {
                    InterstitialLoader.hide()
                    googleInterAd = ad
    ...
    fun runMetaInterstitial(
        ...
        if (context is Activity) InterstitialLoader.show(context, isLoader)
            ...
            inter.buildLoadAdConfig()
                .withAdListener(object : InterstitialAdListener {
                    ...
                    override fun onAdLoaded(ad: Ad?) {
                        ...
                        if (context is Activity) InterstitialLoader.hide()
                        try {
                            inter.show()
