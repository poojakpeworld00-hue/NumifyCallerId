 ...
 *
 * The country is detected **once** — AdHostActivity resolves it early via
 * [resolveGeoFromIp] and stores it in [SettingsVault.homeCountryIso]. Every other
...
object RegionProbe {
    ...
    private const val TAG = "RegionProbe"
     ...
     *  1. [SettingsVault.homeCountryIso] — user pick or AdHostActivity's detection.
     *  2. [SettingsVault.geoCountryIso]  — our own previously-cached IP result.
    ...
    suspend fun detectCountry(context: Context): GeoCountry? {
        ...
        if (BuildConfig.DEBUG) {
            validCountry(DEBUG_FORCE_ISO)?.let { forced ->
                EdgeInsets.log(TAG, "DEBUG force country=$forced (cache/network bypassed)")
        ...
        if (cached != null) {
            EdgeInsets.log(TAG, "reuse cached country=$cached (no IP call)")
        ...
        EdgeInsets.log(TAG, "cached detected country=${geo.iso} for app-wide reuse")
        return geo
    }
    ...
    private suspend fun detectCountryFromIp(): GeoCountry? = withContext(Dispatchers.IO) {
        EdgeInsets.log(TAG, "no cache → resolveGeoFromIp()")
        val location = resolveGeoFromIp()
        if (location == null) {
            EdgeInsets.log(TAG, "resolveGeoFromIp() returned null → null")
            return@withContext null
        }
        EdgeInsets.log(TAG, "location: country=${location.country} code=${location.countryCode}")

        val iso = location.countryCode?.takeIf { it.length == 2 }?.uppercase()
        if (iso == null) {
            EdgeInsets.log(TAG, "no valid countryCode → null")
            return@withContext null
        }
        val dial = CountryCatalog.dialOf(iso).orEmpty()
        EdgeInsets.log(TAG, "resolved country=$iso dial=$dial")
        GeoCountry(iso, dial)
    }
}
