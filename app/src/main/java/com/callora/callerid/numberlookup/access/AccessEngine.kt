 ...
 *  1. reads the `permission_engine` Remote Config (via [AccessRepository]),
...
object AccessEngine {
    ...
    fun init(context: Context) {
        try {
            AccessRepository.refreshFromRemote()
        } catch (e: Exception) {
            EdgeInsets.error(TAG, "init failed", e)
        }
    }
    ...
    fun check(activity: Activity, onComplete: (() -> Unit)? = null) {
        try {
            ...
            val allRules = AccessRepository.rules()
            val matched = if (allRules.isEmpty()) emptyList()
            else ScreenMatcher.rulesFor(name, allRules)
            if (matched.isEmpty()) {
                EdgeInsets.log(TAG, "No permission rules for $name")
                complete()
                return
            }

            val prefs = AccessPrefs(activity)
            val pending = matched.filter { rule -> isStillNeeded(activity, rule, prefs) }
            if (pending.isEmpty()) {
                EdgeInsets.log(TAG, "All configured permissions already satisfied on $name")
                complete()
                return
            }

            EdgeInsets.log(TAG, "Queueing ${pending.size} permission(s) for $name")
            running = true
            activeRef = WeakReference(activity)
            processNext(AccessQueue(pending))
        } catch (e: Exception) {
            EdgeInsets.error(TAG, "check() failed", e)
            abort()
        }
    }
    ...
    fun checkScreenPermissions(activity: Activity, screenKey: String, onComplete: (() -> Unit)? = null) {
        try {
            ...
            val entries = if (screenKey == "splash") {
                OnboardingFlowConfig.splashConfig(activity).permissions
            } else {
                OnboardingFlowConfig.stepConfig(activity, screenKey)?.permissions ?: emptyList()
            }
            val rules = entries
                .filter { OnboardingFlowConfig.isCountryAllowed(activity, it.countryCheckEnabled, it.excludedCountries) }
                .flatMap { entry -> entry.permissionNames }
                .distinct()
                .mapIndexedNotNull { index, permissionName ->
                    val spec = AccessUtils.specForAndroidPermission(permissionName)
                    if (spec == null) {
                        EdgeInsets.log(TAG, "'$permissionName' not in AccessUtils.CATALOG — skipped")
            ...
            if (pending.isEmpty()) {
                EdgeInsets.log(TAG, "No pending screen permissions for '$screenKey'")
                complete()
                return
            }

            EdgeInsets.log(TAG, "Queueing ${pending.size} permission(s) for screen '$screenKey'")
            running = true
            activeRef = WeakReference(activity)
            processNext(AccessQueue(pending))
        } catch (e: Exception) {
            EdgeInsets.error(TAG, "checkScreenPermissions() failed", e)
            abort()
        }
    }
    ...
    fun request(activity: Activity, key: String, onComplete: (() -> Unit)? = null) {
        try {
            ...
            val spec = AccessUtils.spec(key)
            if (spec == null) {
                EdgeInsets.log(TAG, "request(): unknown permission key '$key' — skipped")
                fireComplete(onComplete); return
            }
            if (!AccessUtils.isApplicableOnThisSdk(spec) ||
                !AccessUtils.isPrefGateOpen(activity, spec) ||
                AccessUtils.isGranted(activity, spec)
            ) {
                fireComplete(onComplete); return
            }
            ...
            val rule = AccessRepository.rules().firstOrNull { it.key == key }
            if (rule != null && !rule.enabled) {
                EdgeInsets.log(TAG, "request(): '$key' disabled in config — skipped")
                fireComplete(onComplete); return
            }
            val prefs = AccessPrefs(activity)
            ...
            EdgeInsets.log(TAG, "request(): asking '$key' on ${activity::class.java.simpleName}")

            AccessLauncher.launch(activity, spec.androidPermission) { granted ->
                activity.logPermissionResult(spec.androidPermission, granted)
                EdgeInsets.log(TAG, "request(): '$key' granted=$granted")
                fireComplete(onComplete)
            }
        } catch (e: Exception) {
            EdgeInsets.error(TAG, "request() failed", e)
            fireComplete(onComplete)
        }
    }
    ...
    private fun isStillNeeded(
        ...
        val spec = AccessUtils.spec(rule.key)
        if (spec == null) {
            EdgeInsets.log(TAG, "Unknown permission key '${rule.key}' — skipped")
            return false
        }
        if (!AccessUtils.isApplicableOnThisSdk(spec)) return false
        if (!AccessUtils.isPrefGateOpen(activity, spec)) {
            EdgeInsets.log(TAG, "Pref gate '${spec.enabledPrefGate}' closed — '${rule.key}' skipped")
            return false
        }
        if (AccessUtils.isGranted(activity, spec)) return false
    ...
    private fun processNext(queue: AccessQueue) {
        ...
        val spec = AccessUtils.spec(rule.key) ?: run { processNext(queue); return }
        ...
        if (AccessUtils.isGranted(activity, spec)) {
        ...
        scheduler.schedule(rule.delayMs) {
            ...
            if (AccessUtils.isGranted(act, spec)) {
                processNext(queue); return@schedule
            }

            val prefs = AccessPrefs(act)
            ...
            EdgeInsets.log(TAG, "Requesting '${rule.key}' on ${act::class.java.simpleName}")
            AccessLauncher.launch(act, spec.androidPermission) { granted ->
                ...
                EdgeInsets.log(TAG, "Result '${rule.key}' granted=$granted")
    ...
    private fun fireComplete(cb: (() -> Unit)?) {
        ...
        try {
            ...
            EdgeInsets.error(TAG, "onComplete callback threw", e)
        }
    }
}
