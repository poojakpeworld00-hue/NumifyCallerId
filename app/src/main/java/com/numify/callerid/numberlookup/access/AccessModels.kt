 ...
 * [AccessSpec] to [AccessUtils.CATALOG] and reference its key from
...
data class AccessSpec(
    ...
    val minSdk: Int,
    /**
     * Optional business gate: an `AdsPrefStore` boolean key that must be `true`
     ...
     * deliver a core signal (see the note on `phone_state` in [AccessUtils]).
     */
